...
#else
// This actually comes via the command line in the Makefile
#define F_CPU 16000000

// Unix timers instead of the builtin AVR timer/interrupt system
#include <signal.h>
#include <time.h>
#include <stdio.h>
#include <stdlib.h>
...
#define PULSE_BEGIN_SIGNAL (SIGRTMIN)
#define PULSE_END_SIGNAL (SIGRTMAX)
timer_t pulse_begin_timer;
timer_t pulse_end_timer;


int init_signal_handler(int signum, timer_t *timer, void (*handler)(int)) {
  struct sigaction action;
  action.sa_handler = handler;
  action.sa_flags = SA_RESTART;  // Allow interrupted syscalls to continue automatically
  sigemptyset(&action.sa_mask);  // Allow other signals to interrupt this handler
...
void set_timer(timer_t timerid, long oneshot, long repeat) {
  ...
  if (timer_settime(timerid, 0, &interval, NULL) == -1) {
    printf("Failed to set timer.  Aborting.\n");
    exit(9);
  }

  /*
  struct itimerval interval;
  sigaction(SIGRTMIN, alarm_signal_handler);
  interval.it_interval.tv_sec = 0;
  interval.it_interval.tv_usec = settings.pulse_microseconds;
  interval.it_value.tv_sec = 0;
  interval.it_value.tv_usec = settings.pulse_microseconds;
  printf("Set wakeup for %dus\n", settings.pulse_microseconds);
...
void st_go_idle() 
{
  ...
  set_timer(pulse_begin_timer, 0, 0);
#endif
...
#else
void pulse_begin_handler(int unused)
#endif
{ 
...
#else 
  if (!init_signal_handler(PULSE_BEGIN_SIGNAL, &pulse_begin_timer, pulse_begin_handler)) {
    printf("Failed to init signal handler\n");
    exit(9);
  }

  if (!init_signal_handler(PULSE_END_SIGNAL, &pulse_end_timer, pulse_end_handler)) {
    printf("Failed to init signal handler\n");
    exit(9);
  }
